...
public:
    ...
    void addEdge(int u, int v, int w, int sel) {
        ...
        if (sel == 2) {
            adjList[u].emplace_back(v, w); // Nếu đồ thị vô hướng
...
int main() {
    ...
    for (int i = 0; i < canh; i++)
    {
        ...
        g.addEdge(u, v, w, loai); // Thêm các cạnh vào đồ thị
    }

    cout << "Danh sach ke:" << endl;
    g.display(); // Hiển thị đồ thị
    ...
    do {
        ...
        case 2: {
            ...
            cin >> start;
            g.dijkstra(start); // Gọi thuật toán Dijkstra
            break;
        }
